...
namespace cheat_tools
{
    ...
    weapon_config_t zeus_config = { true, true, false, 70, 100, 100, 50, 50, 0, 0, 0, 1, 0, { } };
    ...
    vector2d calc_spread_angle(int bullets, float recoil_index, int i)
    {
        ...
        float v1 = math::random_float(0.f, 1.f);
        float v2 = math::random_float(0.f, M_PI * 2.f);
        ...
        float inaccuracy = v1 * g_engine_prediction->predicted_inaccuracy;
        float spread = v3 * g_engine_prediction->predicted_spread;

        float cos_v2 = std::cosf(v2);
        float cos_v4 = std::cosf(v4);
        float sin_v2 = std::sinf(v2);
        float sin_v4 = std::sinf(v4);

        return { cos_v2 * inaccuracy + cos_v4 * spread, sin_v2 * inaccuracy + sin_v4 * spread };
    }
    ...
    bool can_hit_hitbox_wrap(const vector3d& start, const vector3d& end, c_csplayer* player, int hitbox, records_t* record, matrix3x4_t* matrix)
    {
        ...
        else
        {
            ...
            auto check_thread = [&result, &start, &end, &min, &max, &studio_box]()
                {
    ...
    bool is_accuracy_valid(c_csplayer* player, point_t& point, float amount, float* out_chance)
    {
        ...
        const int total_seeds = 256; // Замените на фактическое значение
        ...
        for (int i = 0; i < total_seeds; ++i)
        {
            ...
            vector3d end = start + direction * range;
...
namespace resolver
{
        ...
        if (!current->on_ground)
        {
            ...
            else
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    player->eye_angles().y = at_target - 45.f;
                    break;
                case 4:
                    player->eye_angles().y = at_target + 45.f;
                    break;
                case 5:
                    player->eye_angles().y = at_target - 128.f;
                    break;
                case 6:
                    player->eye_angles().y = at_target + 128.f;
                    break;
        ...
        else
        {
            ...
            else
            {
                ...
                if (lby_data.old_lby != lby)
                {
                    ...
                    lby_data.lby_time = current->sim_time + 1.337f;
                    lby_data.old_lby = lby;
                    return;
                }
                ...
                if (misses > 0)
                {
                    ...
                    {
                    case 1:
                        player->eye_angles().y = at_target + 22.8f;
                        break;
                    case 2:
                        player->eye_angles().y = at_target - 125.f;
                        break;
                    case 3:
                        player->eye_angles().y = at_target - 22.8f;
                        break;
                    case 4:
                        player->eye_angles().y = at_target + 55.5f;
                        break;
                    case 5:
                        player->eye_angles().y = at_target - 77.7f;
                        break;
                    case 6:
                        player->eye_angles().y = at_target + 128.f;
                        break;
                    ...
                    case 0:
                        player->eye_angles().y = at_target + 10.f;
                        break;
                    }
                ...
                else
                {
                    ...
                    if (freestanding.available)
                    {
                        if (freestanding.left_damage <= 0 && freestanding.right_damage <= 0)
                        {
                            if (freestanding.right_fraction < freestanding.left_fraction)
                                freestanding.yaw = at_target + 120.f;
                    ...
                    resolver_info.last_stand_angle = player->eye_angles().y - 10.f;
                }
            }
        }
    ...
    void onshot(c_csplayer* player, records_t* record)
    {
        const int num_records = info.size();
        float shoot_time = -1.f;
        float last_normal_pitch;
        ...
        int tick_delta = std::abs(math::time_to_ticks(shoot_time) - math::time_to_ticks(record->sim_time));

        // this record has a shot on it.
        // edit: supremacy retards dont know how m_fLastShotTime is networked
        if (tick_delta <= 1)
        {
            ...
            {
                std::array< resolver_info_t, 65U >* previous = num_records <= 1 ? nullptr : &info;

                if (previous && !player->dormant())
                {
                    record->eye_angles.x = last_normal_pitch;
                }

            }

        }
        else if (tick_delta > 14)
            last_normal_pitch = record->eye_angles.x;
    }

    void on_fsn_thread(int start, int end)
    {
    ...
    void on_fsn()
    {
        ...
        for (int i = 0; i < num_threads; ++i)
        {
            ...
            int end = (i == num_threads - 1) ? num_records : (i + 1) * records_per_thread;

            threads.emplace_back(on_fsn_thread, start, end);
        }
