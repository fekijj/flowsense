...
{
    bool toggle_condition = g_cfg.binds[sw_b].toggled;
    bool should_slide_condition = g_rage_bot->should_slide;
    bool fake_duck_condition = g_anti_aim->is_fake_ducking();

    if (toggle_condition || should_slide_condition || fake_duck_condition)
    ...
    bool on_ground = g_utils->on_ground();
    bool landing = g_ctx.local->fall_velocity() > 0.f;

    if ((g_cfg.antihit.fakelag_conditions & 1 && standing) ||
        (g_cfg.antihit.fakelag_conditions & 2 && !(standing || !on_ground)) ||
        (g_cfg.antihit.fakelag_conditions & 4 && !on_ground && !landing) ||
...
{
    // Проверяем, активирован ли пинг спайк, есть ли локальный игрок и он жив.
    if (!g_cfg.binds[spike_b].toggled || !g_ctx.local || !g_ctx.local->is_alive())
    {
        // Если не выполняются условия, устанавливаем flipped_state на true и выходим.
        flipped_state = true;
        return;
    }
    ...
    // Проверяем изменение reliable state в netchan.
...
{
    // Если flipped_state установлено в true, возвращаемся.
    if (flipped_state)
    {
        flipped_state = false;
        return;
    ...
    // Рассчитываем количество тиков на основе задержки (latency).
    int ticks = math::time_to_ticks(latency);

    // Вычитаем количество тиков из in_sequence_nr.
...
void c_fake_lag::on_predict_end()
{
    ...
    int max_choked_ticks = static_cast<int>(2.f / interfaces::global_vars->interval_per_tick);
    while (choked_ticks.size() > max_choked_ticks)
    {
