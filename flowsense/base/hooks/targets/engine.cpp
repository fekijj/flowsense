...
namespace tr {
    namespace client_state {
        void __fastcall packet_start(void* ecx, void* edx, int incoming, int outgoing) {
            ...
            // Use a simple for loop to minimize overhead
            for (auto it = cmd.rbegin(); it != cmd.rend(); ++it) {
                if (!it->outgoing) continue;
            ...
            // Directly manipulate the vector to erase old commands
            for (auto it = cmd.begin(); it != cmd.end();) {
                if (outgoing == it->cmd || outgoing == it->prev_cmd || (outgoing > it->cmd && outgoing > it->prev_cmd)) {
                    it = cmd.erase(it);
                }
                else {
                    ++it;
        ...
        void __fastcall packet_end(void* ecx, void* edx) {
            ...
            if (!g_ctx.local || !g_ctx.local->is_alive()) return original(ecx);
            ...
            if (clientstate->clock_drift_mgr.cur_clock_offset == clientstate->clock_drift_mgr.server_tick) {
                ...
                auto correct = std::find_if(g_fake_lag->choked_ticks.begin(), g_fake_lag->choked_ticks.end(),
                    [&ack_cmd](const choked_ticks_t& other_data) { return other_data.cmd == ack_cmd; });
                ...
                if (correct != g_fake_lag->choked_ticks.end()) {
                    if (g_ctx.velocity_modifier > g_ctx.local->velocity_modifier() + 0.1f) {
                        ...
                        if (!weapon || (weapon->item_definition_index() != weapon_revolver && !weapon->is_grenade())) {
  ...
  namespace engine
  {
      int __fastcall list_leaves_in_box(void* ecx, void* edx, const vector3d& mins, const vector3d& maxs, unsigned short* list, int list_max) {
          ...
          if (!info || !info->m_pRenderable) return original(ecx, mins, maxs, list, list_max);

          c_baseentity* entity = info->m_pRenderable->get_i_unknown_entity()->get_base_entity();
          auto player = (c_csplayer*)entity;

          if (!entity || !entity->is_player() || !player->is_alive() || entity == g_ctx.local) {
              return original(ecx, mins, maxs, list, list_max);
          }
    ...
    bool __fastcall send_net_msg(i_net_channel_info* ecx, void* edx, c_net_message& msg, bool force_reliable, bool voice) {
        ...
        if (ecx != interfaces::engine->get_net_channel_info()) return original(ecx, edx, msg, force_reliable, voice);

        if (msg.get_type() == 14) return false;
